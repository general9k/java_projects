...
public class PersonDAO {
    ...
    // Проверка имени для PersonVAlidator
    public Optional<Person> show(int id, String fullName) {
        return jdbcTemplate.query("SELECT * FROM Person WHERE id <> ? AND full_name=?", new Object[]{id, fullName},
                        new BeanPropertyRowMapper<>(Person.class))
                .stream().findAny();
    }
